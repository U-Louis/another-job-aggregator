...
3. Run `npm run fetch-payload -- --conf configs/<profile>.yaml` → save as `fixtures/sample.json`
...
| 3 | `fetch-payload` CLI for Adzuna payload capture | pending |
...
- `adapt(fixture)` — field mapping, salary format, HTML stripping, remote enum
