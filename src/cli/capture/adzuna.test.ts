...
test("buildAdzunaCaptureQuery builds the search URL with auth and filters", () => {
  process.env.ADZUNA_APP_ID = "test-app-id"
  process.env.ADZUNA_APP_KEY = "test-app-key"

  const params = buildAdzunaCaptureQuery({
    country: "fr",
    what: "typescript",
    where: "paris",
    what_exclude: "intern",
...
test("buildAdzunaCaptureQuery omits optional filters when absent", () => {
  process.env.ADZUNA_APP_ID = "test-app-id"
  process.env.ADZUNA_APP_KEY = "test-app-key"

  const params = buildAdzunaCaptureQuery({ country: "fr" })
  const url = new URL(params.url)
...
test("buildAdzunaCaptureQuery requires Adzuna credentials", () => {
  delete process.env.ADZUNA_APP_ID
  delete process.env.ADZUNA_APP_KEY

  assert.throws(
    () => buildAdzunaCaptureQuery({ country: "fr" }),
    /Missing environment variable ADZUNA_APP_ID/,
  )
})

test("buildAdzunaCaptureQuery rejects invalid query shape", () => {
  process.env.ADZUNA_APP_ID = "test-app-id"
  process.env.ADZUNA_APP_KEY = "test-app-key"

  assert.throws(
    () => buildAdzunaCaptureQuery({}),
    /Required/,
  )
})
