...
export function fixturePath(provider: string, profile: string): string {
  return join("src", "sources", "api", provider, "fixtures", `${profile}.json`)
}
...
export async function capturePayloadForSource(
  ...
  const path = fixturePath(source.provider, profile)
